...
export default function ReportesPage() {
  ...
  const porArea = useMemo(() => {
    ...
    del_anio.filter((s:any) => {
      ...
      // Display the centro NAME prominently (code as suffix for disambiguation if needed)
      const label = c?.nombre
        ? c.nombre
        : (cid ? `Centro ${cid}` : "Sin centro")
      const codigo = c?.id || c?.codigo || cid || ""
      const key = codigo || "SIN_CENTRO"
      if (!map[key]) map[key] = { label, codigo, total:0, count:0 } as any
          ...
          <div className="card">
            ...
            {porCuenta.length===0 ? (
              <div style={{padding:60,textAlign:"center",color:"var(--text-3)",fontSize:13}}>
                ...
                <div style={{fontSize:11,marginTop:10,color:"var(--text-3)"}}>
                  Esta gráfica agrupa los renglones de los XML comprobados por cuenta contable.<br/>
                  {comprobantes.length === 0
                    ? "No hay comprobantes registrados aún. Sube facturas XML para empezar."
                    : `Hay ${comprobantes.length} comprobantes en la BD, pero ninguno entra en el filtro actual.`}
                </div>
              </div>
            ) : (
