...
export default function ReportesPage() {
  ...
  const kpis = useMemo(() => {
    ...
    // Anticipos que ya tienen comprobación → excluirlos del total para evitar doble conteo
    // (el monto real es el de la comprobación, no el del anticipo)
  ...
  const porArea = useMemo(() => {
    ...
    del_anio.filter((s:any) =>
      ["liberado","comprobado"].includes(s.status) &&
      !(s.tipo==="anticipo" && _withComp2.has(s.id))
    ).forEach(s => {
      const u = usuarios.find(u => u.id === s.usuario_id)
...
git add .
git commit -m "fix: exclude comprobado anticipos from KPIs to avoid double counting"
git push
echo "✓ Done"