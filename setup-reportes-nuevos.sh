...
export default function ReportesPage() {
  ...
  const [filtroMes,  setFiltroMes]  = useState<string>("todos")  // "todos" | "0".."11"
  const [filtroArea, setFiltroArea] = useState<string>("todos")  // "todos" | centro_id
  ...
  const porArea = useMemo(() => {
    const _withComp2 = new Set(del_anio.filter((s:any)=>s.anticipo_ref).map((s:any)=>s.anticipo_ref))
    const map: Record<string, {label:string,total:number,count:number}> = {}
    del_anio.filter((s:any) => {
      if (!["liberado","comprobado"].includes(s.status)) return false
      if (s.tipo==="anticipo" && _withComp2.has(s.id)) return false
      if (filtroMes !== "todos") {
        const m = new Date(s.fecha).getMonth()
      ...
      const c = centros.find((c:any) => c.id === cid)
  ...
  // ── Ranking por cuenta contable ────────────────────────
  const porCuenta = useMemo(() => {
    // Aggregate spending from comprobantes_cfdi grouped by cuenta
    // Filter by year + month (when set)
    ...
    comprobantes.filter(c => validIds.has(c.solicitud_id)).forEach(c => {
      const key = c.cuenta || "—"
      if (!map[key]) map[key] = {
        cuenta: key,
  ...
  // ── Gastos por usuario (filtros: mes + área) ───────────
  const porUsuario = useMemo(() => {
    const _withComp3 = new Set(del_anio.filter((s:any)=>s.anticipo_ref).map((s:any)=>s.anticipo_ref))
    const map: Record<string, { id:string; nombre:string; iniciales:string; centro:string; total:number; count:number }> = {}
    del_anio.filter((s:any) => {
      if (!["liberado","comprobado"].includes(s.status)) return false
      if (s.tipo==="anticipo" && _withComp3.has(s.id)) return false
      if (filtroMes !== "todos") {
        const m = new Date(s.fecha).getMonth()
        if (String(m) !== filtroMes) return false
      }
      const u = usuarios.find(u => u.id === s.usuario_id)
      if (filtroArea !== "todos") {
        if (String(u?.centro_id || s.centro_id || "") !== filtroArea) return false
      }
      return true
    }).forEach((s:any) => {
      const u = usuarios.find(u => u.id === s.usuario_id)
      if (!u) return
      const c = centros.find((c:any) => c.id === (u.centro_id || s.centro_id))
      if (!map[s.usuario_id]) map[s.usuario_id] = {
        id: s.usuario_id,
        nombre: u.nombre || "—",
        iniciales: u.iniciales || "??",
        centro: c?.codigo || c?.nombre || "—",
        total: 0, count: 0,
      }
      ...
      {tab==="area" && (
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div className="card" style={{padding:12,display:"flex",alignItems:"center",gap:10}}>
            ...
            {filtroMes!=="todos" && (
              <button className="btn sm ghost" onClick={()=>setFiltroMes("todos")}>Limpiar</button>
            )}
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div className="card" style={{padding:12,display:"flex",alignItems:"center",gap:10}}>
            ...
            <span style={{marginLeft:"auto",fontSize:12,color:"var(--text-3)"}}>
              Total: <strong style={{color:"var(--accent)"}}>{fmtMXN(porCuenta.reduce((a,c)=>a+c.total,0))}</strong>
            </span>
          </div>
          ...
          <div className="card">
            ...
            {porCuenta.length === 0 ? (
              <div style={{padding:60,textAlign:"center",color:"var(--text-3)",fontSize:13}}>
                Sin gastos comprobados con cuenta contable para este filtro
              </div>
            ) : (
          ...
          <div className="card" style={{padding:0,overflow:"auto"}}>
            <table className="t" style={{minWidth:600}}>
              <thead><tr><th>#</th><th>Cuenta</th><th>Nombre</th><th className="num">Movimientos</th><th className="num">Monto</th><th className="num">% del total</th></tr></thead>
              <tbody>
                {porCuenta.map((c,i)=>{
      ...
      {tab==="usuario" && (
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div className="card" style={{padding:12,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
            ...
            <span style={{marginLeft:"auto",fontSize:12,color:"var(--text-3)"}}>
              Total: <strong style={{color:"var(--accent)"}}>{fmtMXN(porUsuario.reduce((a,u)=>a+u.total,0))}</strong>
            </span>
          </div>
